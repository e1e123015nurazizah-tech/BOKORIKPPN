...
<div class="mt-4 pb-20" x-data="{ 
    ...
    mode: 'view',
    getKategoriLabel() {
        return 'Pengajuan PPNPN';
    }
}">
    <div class="mb-8 flex justify-between items-end">
        <div>
            <h1 class="text-2xl font-black text-[#1e3a8a] tracking-tight uppercase">Monitoring PPNPN</h1>
            <p class="text-gray-500 mt-1 text-sm italic">
                Layanan: <span class="text-blue-600 font-bold uppercase">Rekon PPNPN</span>
            </p>
        </div>
    </div>
    ...
    <div class="mb-10 bg-blue-50/40 p-6 md:p-8 rounded-[35px] border-2 border-blue-200 shadow-md relative overflow-hidden">
        ...
        <div class="overflow-x-auto rounded-2xl bg-white shadow-sm border border-blue-100 max-h-[400px] overflow-y-auto custom-scrollbar">
            <table class="w-full text-left text-sm border-separate border-spacing-0">
                <thead class="bg-blue-100/50 text-[#1e3a8a] uppercase text-[10px] tracking-widest sticky top-0 z-10 backdrop-blur-md">
                    <tr>
                            ...
                            <div class="flex justify-center items-center gap-2">
                                ...
                                @elseif($item->status === 'Diproses')
                                    @if($isOwner || $isSuperAdmin)
                                        ...
                                        <form action="{{ route('admin.pengajuan.lepas', $item->id) }}" method="POST" class="m-0" onsubmit="return confirm('Yakin ingin melepas tiket ini?');">
                                            @csrf @method('PUT')
                                    ...
                                    @else
                                        <span class="text-[9px] font-bold text-gray-400 uppercase italic">
                                            Diproses {{ $item->admin->nama_lengkap ?? 'PIC' }}
                                        </span>
                                    @endif
                                ...
                                @if($isSuperAdmin || $item->status === 'Menunggu')
                                    <form action="{{ route('admin.pengajuan.destroy', $item->id) }}" method="POST" class="m-0" onsubmit="return confirm('PERINGATAN! Yakin ingin menghapus tiket ini secara permanen? Data akan hilang dari KPPN dan Satker.');">
                                        @csrf @method('DELETE')
    ...
    <div x-show="modalProses" x-cloak role="dialog" aria-modal="true" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" x-transition.opacity>
        <div @click.away="modalProses = false" class="bg-white rounded-[35px] w-full max-w-4xl max-h-[95vh] overflow-y-auto p-8 shadow-2xl transform transition-all custom-scrollbar" x-transition.scale.origin.center>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        <div class="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 shadow-inner">
                            ...
                            {{-- INFO BOX PPNPN (Tidak ada PDF) --}}
                            <div class="mt-8 bg-blue-50 p-4 rounded-2xl border border-blue-100 text-center">
                                <i class="fas fa-info-circle text-blue-500 mb-2 text-xl"></i>
                                <p class="text-[10px] font-bold text-blue-700 uppercase">Pengajuan PPNPN</p>
                                <p class="text-[9px] text-blue-500 italic">Kategori ini tidak memerlukan lampiran berkas PDF.</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-amber-50/50 p-5 rounded-2xl border border-amber-100/50 mt-6">
                            ...
                            <p class="text-xs text-amber-900 leading-relaxed italic" x-text="dataDetail.detail_ppnpn?.catatan_satker || 'Tidak ada catatan tambahan.'"></p>
                        </div>
                    </div>
