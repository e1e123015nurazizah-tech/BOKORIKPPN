...
<div class="mt-4 pb-20">
    <h2 class="text-4xl font-bold text-gray-800 tracking-tight text-[#1e3a8a]">Overview {{ Auth::guard('admin')->user()->nama_lengkap }}</h2>
    <p class="text-gray-500 mt-2 text-lg italic">Data layanan BOKORI seluruh Satker tahun {{ $tahunAktif }}</p>
    ...
    <div class="mt-10">
        <div class="bg-white p-8 rounded-[35px] shadow-sm border border-gray-50">
            <h3 class="font-bold text-[#1e3a8a] mb-8 uppercase tracking-widest text-[10px]">Aktivitas Terakhir Semua Satker</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="text-gray-300 uppercase text-[10px] tracking-widest border-b border-gray-100">
                        <tr>
                            <th class="pb-5 px-4">Nomor Tiket</th>
                            <th class="pb-5 px-4">Layanan</th>
                            <th class="pb-5 px-4">Tanggal</th>
                            <th class="pb-5 px-4 text-center">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @forelse($pengajuanTerakhir as $item)
                        <tr class="hover:bg-blue-50/50 transition-colors">
                            <td class="py-5 px-4 font-bold text-blue-600">#{{ $item->nomor_tiket }}</td>
                            <td class="py-5 px-4 font-bold text-gray-700 text-xs">Rekon {{ $item->kategori_layanan }}</td>
                            <td class="py-5 px-4 text-gray-400 text-xs">{{ \Carbon\Carbon::parse($item->created_at)->format('d M Y') }}</td>
                            <td class="py-5 px-4 text-center">
                                <span class="px-4 py-1.5 rounded-full text-[9px] font-black uppercase 
                                    {{ $item->status == 'Selesai' ? 'bg-green-100 text-green-600' : 
                                       ...
                                       ($item->status == 'Diproses' ? 'bg-blue-100 text-blue-600' : 'bg-amber-100 text-amber-600')) }}">
                                    {{ $item->status }}
                                </span>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="4" class="text-center py-10 text-gray-300 italic font-medium">Belum ada pengajuan masuk.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    new Chart(document.getElementById('lineChart'), {
        ...
        data: {
            ...
            datasets: [
                { label: 'PNS', data: @json($trenPNS), borderColor: '#1e3a8a', backgroundColor: '#1e3a8a', pointRadius: 4, pointHoverRadius: 6, tension: 0.4 },
                { label: 'PPPK', data: @json($trenPPPK), borderColor: '#fbbf24', backgroundColor: '#fbbf24', pointRadius: 4, pointHoverRadius: 6, tension: 0.4 },
                { label: 'POLRI', data: @json($trenPOLRI), borderColor: '#78350f', backgroundColor: '#78350f', pointRadius: 4, pointHoverRadius: 6, tension: 0.4 },
                { label: 'TNI', data: @json($trenTNI), borderColor: '#16a34a', backgroundColor: '#16a34a', pointRadius: 4, pointHoverRadius: 6, tension: 0.4 }
            ]
        },
        options: { 
            ...
            scales: { 
                ...
                x: { grid: { drawOnChartArea: true, color: 'rgba(0,0,0,0.03)' } }
            },
            plugins: { 
                legend: { display: true, position: 'top', labels: { usePointStyle: true, boxWidth: 8 } },
                datalabels: { display: false },
                tooltip: { usePointStyle: true, boxPadding: 6, padding: 12, backgroundColor: 'rgba(15, 23, 42, 0.9)', titleColor: '#ffffff', bodyColor: '#e2e8f0', borderColor: '#334155', borderWidth: 1 }
            }
        }
    });
    ...
    new Chart(document.getElementById('chartGaji'), {
        ...
        data: {
            ...
            datasets: [{ data: [{{ $gajiPNS }}, {{ $gajiPPPK }}, {{ $gajiPOLRI }}, {{ $gajiTNI }}], backgroundColor: ['#1e3a8a', '#fbbf24', '#78350f', '#16a34a'], borderRadius: 8 }]
        },
        options: barOptions
    });
    ...
    new Chart(document.getElementById('chartPpnpn'), {
        ...
        data: {
            labels: ['Baru', 'Batal'],
            datasets: [{ data: [{{ $ppnpnBaru }}, {{ $ppnpnBatal }}], backgroundColor: ['#3b82f6', '#f50b42'], borderRadius: 8 }]
        },
        options: barOptions
    });
    ...
    new Chart(document.getElementById('chartSkpp'), {
        ...
        data: {
            ...
            datasets: [{ data: [{{ $skppPNS }}, {{ $skppTNI }}, {{ $skppPOLRI }}, {{ $skppPPPK }}], backgroundColor: ['#1e3a8a', '#16a34a', '#78350f', '#fbbf24'], borderRadius: 8 }]
        },
        options: barOptions
    });
</script>
