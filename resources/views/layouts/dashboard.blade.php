...
<body class="bg-[#f8fbff] font-sans text-gray-800">
    
    <div class="min-h-screen w-full relative">
        
        <aside class="w-72 bg-[#42a5f5] text-white hidden md:flex flex-col shadow-xl z-40 h-screen fixed top-0 left-0">
            ...
            <nav class="flex-1 mt-4 px-4 space-y-2 overflow-y-auto sidebar-scroll pb-10">
                @php
                ...
                @if($role !== 'approver')
                <a href="{{ $targetRoute }}" class="flex items-center py-4 px-6 hover:bg-white/10 rounded-xl transition-all {{ Request::is($activeClass) ? 'bg-white/20 font-bold shadow-inner' : '' }}">
                ...
                <div x-data="{ open: {{ Request::is('*pengajuan*') ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="w-full flex items-center justify-between py-3 px-6 hover:bg-white/10 rounded-xl transition-all group" :class="open ? 'bg-white/10' : ''">
                ...
                <div x-data="{ openMonitoring: {{ Request::is('*monitoring*') ? 'true' : 'false' }} }" class="mt-2">
                    <button @click="openMonitoring = !openMonitoring" 
                            class="w-full flex items-center justify-between px-6 py-3 text-white font-medium rounded-xl hover:bg-white/10 transition-all group"
                ...
                <a href="{{ route('satker.profil') }}" class="flex items-center mt-2 py-3 px-6 hover:bg-white/10 rounded-xl transition-all {{ request()->routeIs('satker.profil') ? 'bg-white/20 font-bold shadow-inner' : '' }}">
                ...
                <div class="px-6 pt-6 pb-2 text-[10px] font-bold text-white/50 uppercase tracking-widest">Administrator</div>
                
                {{-- Sembunyikan menu ini jika role adalah approver --}}
                @if($role !== 'approver')
                    @if($role === 'superadmin')
                    <a href="{{ route('admin.kelola-admin.index') }}" class="flex items-center py-3 px-6 hover:bg-white/10 rounded-xl transition-all {{ request()->routeIs('admin.kelola-admin.*') ? 'bg-white/20 font-bold shadow-inner' : '' }}">
                        <i class="fas fa-user-tie mr-4 text-lg"></i>
                        <span class="text-sm font-semibold">Kelola Admin</span>
                    </a>
                    @endif

                    <a href="{{ route('admin.satker.index') }}" class="flex items-center mt-2 py-3 px-6 hover:bg-white/10 rounded-xl transition-all {{ request()->routeIs('admin.satker.*') ? 'bg-white/20 font-bold shadow-inner' : '' }}">
                        <i class="fas fa-users mr-4 text-lg"></i>
                        <span class="text-sm font-semibold">Kelola Satker</span>
                    </a>

                    @if($role === 'superadmin')
                    <a href="{{ route('admin.maintenance.index') }}" class="flex items-center mt-2 py-3 px-6 hover:bg-red-500/20 rounded-xl transition-all {{ request()->routeIs('admin.maintenance.*') ? 'bg-red-600 font-bold shadow-inner' : '' }}">
                ...
                <a href="{{ route('admin.profil') }}" class="flex items-center mt-2 py-3 px-6 hover:bg-white/10 rounded-xl transition-all {{ request()->routeIs('admin.profil') ? 'bg-white/20 font-bold shadow-inner' : '' }}">
                ...
                <div class="pt-10">
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="flex items-center py-4 px-6 hover:bg-red-500 rounded-xl transition-all w-full text-left group">
        ...
        <div class="flex flex-col min-h-screen md:ml-72 transition-all duration-300">
            <header class="h-24 bg-white flex items-center justify-between px-12 shadow-md sticky top-0 z-30 flex-shrink-0">
                <div class="flex-shrink-0">
                    <h1 class="text-5xl font-black text-blue-600 uppercase tracking-tighter">BOKORI {{ $tahunAktif }}</h1>
                </div>

                
                <div class="flex items-center gap-6">
                    <div class="relative group z-50">
                        <button class="flex items-center gap-2 bg-white text-[#1e3a8a] px-4 py-2 rounded-xl font-bold border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-all shadow-sm text-sm">
                            <i class="fas fa-calendar-alt text-blue-500"></i> 
                            Tahun: {{ $tahunAktif }} 
                            <i class="fas fa-chevron-down text-[10px] ml-1 text-gray-400 transition-transform group-hover:rotate-180"></i>
                        </button>
                    ...
                    <a href="{{ $profileRoute }}" class="flex items-center gap-4 text-right hover:bg-blue-50 p-2 pr-4 rounded-full transition-all cursor-pointer group border border-transparent hover:border-blue-100">
                        <div>
                            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1 group-hover:text-blue-500 transition-colors">
                                {{ $roleLabel }}
                            </p>
                            <p class="text-xl font-bold text-[#1e3a8a] leading-none">{{ $name }}</p>
                            <p class="text-[10px] text-gray-400 mt-1 font-medium italic">
                                {{ $isAdmin ? 'NIP: ' . $user->nip : 'Kode: ' . $user->kode_satker }}
                            </p>
                        </div>
                        
                        <div class="w-14 h-14 rounded-full border-2 border-blue-100 shadow-sm overflow-hidden bg-blue-500 flex items-center justify-center group-hover:border-blue-400 group-hover:shadow-md transition-all group-hover:scale-105">
