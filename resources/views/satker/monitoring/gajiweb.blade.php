...
<div class="mt-4 pb-20">
    <div class="mb-8">
        <h1 class="text-2xl font-black text-[#1e3a8a] tracking-tight">MONITORING REKON GAJIWEB</h1>
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-8 overflow-hidden">
        <form action="{{ route('satker.monitoring.gajiweb') }}" method="GET" class="flex flex-wrap lg:flex-nowrap gap-4 items-end w-full">
            ...
            <div class="flex-1 min-w-[150px]">
                ...
                <select name="jenis_pegawai" onchange="this.form.submit()" class="w-full bg-gray-50 border border-gray-200 text-gray-700 py-3 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-bold text-[#1e3a8a] mb-2">Kategori ADK</label>
                <select name="kategori_adk" onchange="this.form.submit()" class="w-full bg-gray-50 border border-gray-200 text-gray-700 py-3 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium text-xs">
                    <option value="">-- Semua Kategori --</option>
                    <option value="ADK Gaji (.gpp)" {{ request('kategori_adk') == 'ADK Gaji (.gpp)' ? 'selected' : '' }}>ADK Gaji (.gpp)</option>
                    <option value="ADK Penyamaan Data (.pgw / .kgw)" {{ request('kategori_adk') == 'ADK Penyamaan Data (.pgw / .kgw)' ? 'selected' : '' }}>ADK Penyamaan Data (.pgw / .kgw)</option>
                    <option value="ADK Pegawai Baru (.krm / .bru / .kkk)" {{ request('kategori_adk') == 'ADK Pegawai Baru (.krm / .bru / .kkk)' ? 'selected' : '' }}>ADK Pegawai Baru (.krm / .bru / .kkk)</option>
                    <option value="ADK Kelengkapan SK (.sk)" {{ request('kategori_adk') == 'ADK Kelengkapan SK (.sk)' ? 'selected' : '' }}>ADK Kelengkapan SK (.sk)</option>
                    <option value="ADK Perbaikan NIP (.kor)" {{ request('kategori_adk') == 'ADK Perbaikan NIP (.kor)' ? 'selected' : '' }}>ADK Perbaikan NIP (.kor)</option>
                    <option value="ADK Pegawai Baru (.krm / .bru / .kkk) yang masih gantung / belum disetujui" {{ request('kategori_adk') == 'ADK Pegawai Baru (.krm / .bru / .kkk) yang masih gantung / belum disetujui' ? 'selected' : '' }}>Pegawai Baru (Gantung/Belum Setuju)</option>
                    <option value="ADK Kelengkapan SK (.sk) yang masih gantung / belum disetujui" {{ request('kategori_adk') == 'ADK Kelengkapan SK (.sk) yang masih gantung / belum disetujui' ? 'selected' : '' }}>Kelengkapan SK (Gantung/Belum Setuju)</option>
                    <option value="ADK Perbaikan NIP (.kor) yang masih gantung / belum disetujui" {{ request('kategori_adk') == 'ADK Perbaikan NIP (.kor) yang masih gantung / belum disetujui' ? 'selected' : '' }}>Perbaikan NIP (Gantung/Belum Setuju)</option>
                </select>
            </div>
            ...
            <div class="flex flex-none gap-3">
                ...
                <a href="{{ route('satker.monitoring.gajiweb') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-3 px-6 rounded-xl transition-all flex items-center justify-center">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left border-collapse min-w-[2000px]">
                <thead class="sticky top-0 z-20 bg-white">
                    <tr class="bg-gray-50 border-b border-gray-100 uppercase text-[11px] font-black text-gray-500 tracking-wider">
                        ...
                        <th class="px-6 py-5 text-center">Berkas ADK</th>
                        <th class="px-6 py-5">Jenis Proses</th>
                        <th class="px-6 py-5">Kategori ADK</th>
                    ...
                    <tr class="transition-all {{ $d->status == 'Ditolak' ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-blue-50/30' }}">
                        ...
                        <td class="px-6 py-5 text-center whitespace-nowrap">
                            <span class="px-3 py-1 bg-teal-100 text-teal-800 rounded-lg text-[10px] font-black uppercase">
                                {{ $d->detailGaji->bulan_periode ?? '-' }}
                            </span>
                        </td>
                        <td class="px-6 py-5 text-xs font-bold text-gray-400 whitespace-nowrap uppercase">
                            {{ $d->detailGaji->jenis_pegawai ?? '-' }}
                        </td>
                        <td class="px-6 py-5 text-center whitespace-nowrap">
                            @php
                                $namaFile = $d->detailGaji->file_adk ?? $d->detailGaji->file_kelengkapan ?? null;
                            @endphp
                        ...
                        <td class="px-6 py-5 text-[10px] font-bold text-gray-400 whitespace-nowrap uppercase">
                            {{ $d->detailGaji->jenis_proses ?? '-' }}
                        </td>
                        
                        <td class="px-6 py-5 text-xs font-black text-blue-600 whitespace-nowrap uppercase">
                            {{ $d->detailGaji->kategori_adk ?? '-' }}
                        </td>
                        
                        <td class="px-6 py-5 text-xs text-gray-500 italic min-w-[200px]">
                            {{ $d->detailGaji->catatan_satker ?? '-' }}
                        </td>
                        ...
                        <td class="px-6 py-5 text-xs min-w-[250px]">
                            @if($d->status == 'Ditolak')
                                <span class="text-red-700 font-bold leading-snug uppercase">{{ $d->catatan ?? '-' }}</span>
                            @else
                                <span class="text-gray-500 leading-snug uppercase">{{ $d->catatan ?? '-' }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="13" class="px-6 py-32 text-center text-gray-300 font-bold italic">Data tidak ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
